...
\section{The deterministic limit}
...
\label{BD_for_kimura}
\begin{aligned}
b_K(x|\psi) &= \mu b_{\textrm{mut}}(x|\psi) + b_{\textrm{ind}}(x|\psi)\psi(x,t)\\
d_K(x|\psi) &= d_{\textrm{ind}}(x|\psi)\psi(x,t)
\end{aligned}
\end{equation}
Here, $ b_{\textrm{mut}}(x|\psi)$ describes birth due to mutations, and $\mu \geq 0$ is a mutation rate. The functions $b_{\textrm{ind}}(x|\psi)$ and $d_{\textrm{ind}}(x|\psi)$ describe the per-capita birth rate and death rate of type $x$ in a population $\psi$. For example,  $b_{\textrm{ind}}(x|\psi)$ may incorporate the effects of mate choice in the sexual case or intrinsic duplication rate in the asexual case, and $d_{\textrm{int}}(x|\psi)$ may model death due to intraspecific competition for resources. Substituting equation \eqref{BD_for_kimura} into \eqref{deterministic_traj}, we obtain
\begin{equation}
