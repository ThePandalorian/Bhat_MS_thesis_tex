...
\end{equation}
where I have defined $w(x|\psi) \coloneqq b^{\textrm{(ind)}}(x|\psi) - d^{\textrm{(ind)}}(x|\psi)$, the (Malthusian) `fitness' of the phenotype $x$. To track population numbers and trait frequencies, let us define as before, the scaled population size and trait frequency as
\begin{equation}
\label{pop_size_and_freq_for_kimura}
\begin{aligned}
N_K(t) &\coloneqq \int\limits_{\mathcal{T}}\psi(x,t)dx = \frac{1}{K} \int\limits_{\mathcal{T}}\nu(x,t)dx\\
p(x,t) &\coloneqq \frac{\psi(x,t)}{N_K(t)} = \frac{\nu(x,t)}{\int\limits_{\mathcal{T}}\nu(y,t)dy}
\end{aligned}
\end{equation}
...
&=\left(\int\limits_{\mathbb{R}}|m(x)|dx\right)\left(\int\limits_{\mathbb{R}}|\psi(z,t)|dz\right) < \infty
\end{align*}
...
We can now use the same trick we used in deriving \eqref{nD_Price_time_dependent} from \eqref{nD_replicator_mutator}. By multiplying both sides of equation \eqref{cts_replicator_mutator} by a type level quantity\footnote{This is now a real function $f(x,t):\mathcal{T} \times [0,infty) \to \mathbb{R}$ of two variables, the trait value and time. I assume this function is nice enough for all the below operations to make sense} $f(x,t)$ and integrating over the trait space, we obtain
\begin{align}
