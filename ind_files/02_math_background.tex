...
\end{equation*}
\textcolor{red}{\st{Using this, and the fact that the future path of the Brownian motion itself is independent of its history, one can derive the following `multiplication table' for manipulating products of formal expressions containing stochastic integrals with respect to the Brownian motion}} One important but not immediately obvious consequence of \textcolor{red}{\st{this table}} \textcolor{blue}{the definition of stochastic integrals and the Brownian motion} is that we can no longer rely on the normal rules of calculus when dealing with stochastic integrals. In regular calculus, if we had a continuous quantity $x(t)$ satisfying
\begin{equation*}
	\frac{dx}{dt} = f(x) + g(x)
\end{equation*}
