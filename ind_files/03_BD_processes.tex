...
\begin{aligned}
b^{(K)}_i(\mathbf{x}) &= \mu Q_i(\mathbf{x}) + b^{(\textrm{int})}_{i}(\mathbf{x})x_i\\
...
&- \frac{1}{K}\underbrace{\frac{1}{N(t)}\left[(\tau_i(\mathbf{x}) - \overline{\tau})p_i + \mu\left\{Q_i(\mathbf{p}) - p_i\left(\sum\limits_{j=1}^{m}Q_j(\mathbf{p})\right)\right\}\right]dt}_{\substack{\text{Directional finite population effects}\\\text{due to differential turnover rates}}}\\
...
\end{aligned}
\end{equation}
where $B^{(1)}_t,B^{(2)}_t, \ldots, B^{(m)}_t$ are $m$ independent one-dimensional standard Brownian motion processes. The first term in this expression is the same as equation \eqref{nD_replicator_mutator} and describes directional changes in the population composition due to `classical' evolutionary forces such as selection and mutation. As we can see, the replicator-mutator equation is generically satisfied for all birth-death processes that are in the (very general) functional forms \eqref{nD_functional_forms_for_replicator}. The second term is an additional directional force on population composition that acts exactly analogously to the replicator equation but in the opposite direction (A higher relative $\tau_i$ leads to a lower frequency), and furthermore, is particular to finite populations. This term describes a biasing effect due to differential turnover rates and can intuitively be understood in the following sense: If a type $i$ has a higher $\tau_i$, it experiences greater turnover due to a generally higher birth and death rate and thus greater demographic stochasticity. Types with a higher $\tau_i$ thus tend to be eliminated by a stochastic analog of selection because they experience more chance events (births and deaths) in a given time period (notice the minus sign before the second term in \eqref{nD_eqn_for_frequencies}). This effect is less visible if the total population size is higher because larger populations generally experience less stochasticity, and this is reflected in the $1/N$ factor in this term. This stochastic analog of selection for reduced turnover rates is the force responsible for the `reversal of the direction of deterministic selection' observed in previous studies \citep{constable_demographic_2016, mcleod_social_2019} and is an effect that is only seen in finite populations. Note that types that tend to increase the \emph{total} population size (such as altruists in evolutionary theory and mutualists in ecological communities) experience a smaller negative effect due to differential turnover rates than types that do not facilitate such an increase (such as cheaters and highly competitive species), which could explain why mutualists and cooperators routinely persist for long periods in finite population models despite extinction being predicted in infinite population models. The fact that the entire term is multiplied by $(KN(t))^{-1}$, however, suggests that the effect of this force is weak for medium to large populations, which explains why the aforementioned persistence of cooperators is often only observed in restrictive sounding conditions such as quasi-neutrality, rapid attraction to a slow manifold, or a weak selection limit. In all three of these cases, the first term on the RHS of \eqref{nD_eqn_for_frequencies} becomes identically 0 and therefore no longer contributes to the total dynamics, thus revealing the effects of differential turnover rate.\\
Finally, the last term describes non-directional stochastic effects due to fluctuations and has a `spreading effect'. To the highest order in $\sqrt{K}$, the effect of these fluctuations can be described through the weak noise approximation, as we explain below. However, the $1/K$ factor in the second term of \eqref{nD_eqn_for_frequencies} means that the effect of differential turnover rates disappears in this weak noise approximation  (and also in other such approximate treatments, such as the Chemical langevin).
...
\label{BD_for_kimura}
\begin{aligned}
    b_K(x|\psi) &= \mu b_{\textrm{mut}}(x|\psi) + b_{\textrm{int}}(x|\psi)\psi(x,t)\\
...
\end{equation}
where we have defined $w(x|\psi) \coloneqq b_{\textrm{int}}(x|\psi) - d_{\textrm{int}}(x|\psi)$, which can be thought of as the (Malthusian) `fitness' of the phenotype $x$. To track population numbers and trait frequencies, we follow the approach of \citep{week_white_2021} and define
\begin{equation}
