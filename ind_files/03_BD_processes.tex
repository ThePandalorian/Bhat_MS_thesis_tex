...
\end{equation*}
Naively, we may expect the same logic to still hold true for one-dimensional It\^o processes of the form \eqref{ito_SDE_diff} with $gdt$ simply being replaced by $GdB_t$ on the RHS. However, this intuition is not true. The correct relation is given by 
\emph{It\^o's formula}\footnote{It\^o's formula also additionally requires $h \in C^2(\mathbb{R})$, meaning that $h$ is continuous and the first and second derivatives of $h$ exist and are also continuous}, which states:
\begin{equation*}
dh(X_t) = h'(X_t)F(X_t)dt + h'(X_t)G(X_t)dB_t  + \frac{h''(X_t)}{2}G^2(X_t)dt 
\end{equation*}
where the presence of the stochastic term has added an entire extra term $G^2(X_t)h''(X_t)/2$ that does not exist in the deterministic setting, in some sense because the Brownian motion `wiggles too erratically' over time. Using It\^o's formula and some simple algebra, one can then show that if a process $X_t$ taking values in $\mathbb{R}$ satisfies the SDE \eqref{ito_SDE_diff}, then the probability density $P(x,t)$ of finding the process in a state $x \in \mathbb{R}$ at time $t$ (if it exists) satisfies the PDE
\begin{equation}
\label{ito_FPE}
