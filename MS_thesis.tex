\documentclass[twoside, 12pt]{iiser-thesis}

%Bio dept wants all font in Arial (ew)
%Will keep it commented until the v end for my own sanity. I refuse to actively change a tex file to Arial permanently. Change compiler to XeLaTeX or LuaLaTeX for this to work
%\usepackage{fontspec}
%\setmainfont{Arial}

%my own list of packages
\input{head}

%Add fancy headers with chapter/section titles
\usepackage{fancyhdr}
\setlength{\headsep}{0.9in} %add spacing between header and body text

%this file contains the bibliography
\addbibresource{refs.bib}

%%%%%%%%%%%%%%%%%%%
% Packages/Macros %
%%%%%%%%%%%%%%%%%%%
\usepackage{fullpage}
\setlength{\parskip}{1em}

\usepackage{setspace}
\onehalfspacing

\setcounter{tocdepth}{3}

\catcode`\@=11
\catcode`\@=12

%%%%%%%%%%%%
% Document %
%%%%%%%%%%%%

\title{Eco-evolutionary dynamics of finite populations from first principles}
\author{Ananda Shikhara Bhat}

%Details of supervisors and experts
\supervisor{Vishwesha Guttal}
\cosupervisor{Rohini Balakrishnan}
\department{Centre for Ecological Sciences, Indian Institute of Science} \reader{Sutirth Dey} %Expert

\dedication{This thesis is dedicated to ?}

%Details of graduation date
\graduationyear{2023}
\academicyear{2022-2023}
\graduationmonth{May}

%The abstract goes here. IISER guidelines say it should be <= 250 words.
\thesisabstract{
Some central questions of population biology relate to the number of distinct variants of an entity that can emerge and coexist in a closed population. Examples include the study of standing genetic variation (alleles), polymorphisms (genotypes/phenotypes), and sympatric speciation (species). Historically, these questions have often been studied through phenomenological models formulated for infinitely large populations. In this thesis, I use ideas from statistical physics to analytically describe evolving populations from biological first principles. Starting from a density-dependent ‘birth-death process’ describing an arbitrary closed population of individuals with discrete traits, I derive a set of stochastic differential equations for how trait frequencies change over time. Well-known relations such as the replicator-mutator equation and Fisher’s fundamental theorem are recovered in the infinite population limit. I then extend this theory to one-dimensional quantitative traits through a ‘stochastic field theory’ that yields frameworks such as Kimura's continuum-of-alleles and adaptive dynamics as special cases. I make no assumptions other than density-dependence and the impossibility of infinite growth, and thus, my derivation is almost universally valid. The resultant equations reveal a new directional evolutionary force, ‘noise-induced selection’, that is particular to finite populations and has been overlooked in standard mathematical formulations of evolution. Noise-induced selection can reverse the direction of evolution predicted by infinite-population models, with implications for simulation studies and real world populations. My work thus derives a set of fundamental equations showcasing the common stochastic underpinnings of several formal structures in population biology and predicts a new evolutionary force unique to finite populations.
}

\acknowledgments{Not more than 250 words}
\allowdisplaybreaks %allow align envts to go across pages   


\begin{document}

\pagestyle{plain}
\thesisfront
\listoftables
\addcontentsline{toc}{chapter}{List of Tables}
\listoffigures
\addcontentsline{toc}{chapter}{List of Figures}

\part{Background}


%Implement the fancy headers for everything from here on out
\pagestyle{fancy}
\fancyhf{}
\fancyhead[LE,RO]{\thepage}
\fancyhead[RE]{\rule[-2ex]{0pt}{4ex}\footnotesize\itshape\nouppercase{\leftmark}}
\fancyhead[LO]{\rule[-2ex]{0pt}{4ex}\footnotesize\itshape\nouppercase{\rightmark}}
\setlength{\headheight}{18pt} % as requested by fancyhdf
\renewcommand{\headrulewidth}{0pt}

\renewcommand{\chaptermark}[1]{\markboth{\chaptername \ \thechapter \ :\ #1}{}}
\renewcommand{\sectionmark}[1]{\markright{\thesection. \ \ #1}{}}

\chapter{Introduction}
\input{ind_files/01_Introduction}
\chapter{Adaptive diversification: a crash course}\label{chap_AD}
\input{ind_files/02_Foundations}

\part{Theory}
\chapter{Population dynamics from stochastic first principles}\label{chap_BD}
\input{ind_files/03_BD_processes}
\chapter{Stochastic field equations for the evolution of quantitative traits}\label{chap_infD_processes}
\input{ind_files/04_field_eqns}

\part{Discussion \& Outlook}
\chapter{A unified view of population dynamics}\label{chap_unification}
\input{ind_files/05_unification_discussion}
%\chapter{Towards a stochastic evolutionary theory}\label{chap_stochasticity}
%\input{ind_files/07_stochastic_discussion}
% \chapter{}
% \input{ind_files/chapter_08}
% \chapter{} <-- uncomment and add as required
% \input{ind_files/chapter_09}

%You need the below commands to elevate the appendix to 'Part' level in the TOC
%from https://tex.stackexchange.com/a/286452
\cleardoublepage
\phantomsection
\appendix
\addcontentsline{toc}{part}{Appendices}

%make sure the title is 'Appendix' and not 'Chapter'
\renewcommand{\chaptername}{Appendix} 
\chapter{Deriving the Fokker-Planck equations for It\^{o} SDEs}\label{App_SDE_FPE}
\input{ind_files/99_Appendix_A}
\chapter{Deriving stochastic trait frequency dynamics using It\^{o}'s formula}\label{App_density_to_freq}
\input{ind_files/99_Appendix_B}
\chapter{A Price-like equation for the variance of a type-level quantity}\label{App_stoch_var_eqns}
\input{ind_files/99_Appendix_C}
\chapter{Some Examples}\label{App_examples}
\input{ind_files/99_Appendix_D_examples}

\cleardoublepage
\phantomsection
\printbibliography
\addcontentsline{toc}{part}{References}
\end{document}
